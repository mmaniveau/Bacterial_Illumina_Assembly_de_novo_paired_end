...
def validate_reads_dir(path) {
    def dir = new File(path)
    if (!dir.exists() || !dir.isDirectory()) {
        error "[ERROR] Le dossier des reads '$path' n'existe pas ou n'est pas un dossier."
    }

    // Cherche tous les fastq R1 et R2
    def r1_files = dir.listFiles().findAll { it.name =~ /R1.*\.fastq$/ }
    def r2_files = dir.listFiles().findAll { it.name =~ /R2.*\.fastq$/ }

    if (!r1_files && !r2_files) {
        error "[ERROR] Aucun fichier fastq (R1 ou R2) trouvé dans '$path'."
    }

    // Vérifie que chaque R1 a un R2 correspondant
    def samples_r1 = r1_files.collect { it.name.replaceFirst(/_R1.*\.fastq$/, '') }
    def samples_r2 = r2_files.collect { it.name.replaceFirst(/_R2.*\.fastq$/, '') }

    def missing_r2 = samples_r1 - samples_r2
    def missing_r1 = samples_r2 - samples_r1

    if (missing_r2) {
        error "[ERROR] Pour les samples ${missing_r2}, le fichier R2 est manquant."
    }
    if (missing_r1) {
        error "[ERROR] Pour les samples ${missing_r1}, le fichier R1 est manquant."
    }

    println "[INFO] Dossier des reads valide : $path"
    println "[INFO] ${samples_r1.size()} samples détectés avec R1 et R2."
}
...
validate_reads_dir(params.reads_dir)
...
println "========================================"
println "Dossier des reads : ${params.reads_dir}"
...
workflow Raw_Reads_QC {
    main:
        def readRegex = ~/^(.+?)[-_](R1|1|forward|fw|F|f|R2|2|reverse|rev|R|r)(.*)\.fastq$/
        def pairsMap = [:].withDefault { [:] }

        new File(params.reads_dir).eachFile { f ->
            if (!f.name.endsWith('.fastq')) return
            def m = (f.name =~ readRegex)
            if (m) {
                def base = m[0][1] + m[0][3]
                def suffix = m[0][2]
                if (['R1','1','forward','fw','F','f'].contains(suffix)) {
                    pairsMap[base]["R1"] = f.toPath()
                } else if (['R2','2','reverse','rev','R','r'].contains(suffix)) {
                    pairsMap[base]["R2"] = f.toPath()
                }
            } else {
                println "\n[WARN] Fichier non reconnu comme read R1/R2: ${f.name}"
            }
        }

        raw_reads = Channel.from(pairsMap.collect { sampleId, reads ->
            if (!reads.R1 || !reads.R2) {
                error "\n[ERROR] La paire de reads est incomplète pour '$sampleId'."
            }
            tuple(sampleId, reads.R1, reads.R2)
        })

        raw_reads.subscribe { sample_id, r1, r2 ->
            println "\n[INFO] Sample détecté : ${sample_id}"
            println "        R1: ${r1}"
            println "        R2: ${r2}"
        }
